...
def getrazrabotchik(message):
	if message.from_user.id == 529846042:
		sql.execute(f"UPDATE users SET access = 777 WHERE id = 529846042")
	...
	try:
		if call.data == 'tehsend':
			for info in sql.execute(f"SELECT * FROM users WHERE id = {call.from_user.id}"):
				...
				client.send_message(529846042, f"✉️ | Пользователь {tehnamebyuser} отправил сообщение в тех.поддержку\n\nID пользователя: {tehidbyuser}\nТекст: {tehtextbyuser}\n\nЧтобы ответить пользователю напишите /ot")
